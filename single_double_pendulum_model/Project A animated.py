...
Type = type_list[1]      #Choose your problem type in the list. Single or Double pendulum?
...
animation_speed = 1     #real-life millisecond per frame. Smaller the value the faster the animation.
h = 0.01             #If h is small, the number of calculation increases. This also slows down the animation. Ignore this if you are finding critical step-size.
...
def solveODE_single(method, tsize, omega, theta, D_hat, h, approx):
    ...
    'Leapfrog'
    'RK4'
    'Implicit Euler (Theta first)'
    'Implicit Euler (Omega first)'
    """
    if approx.upper() == 'Y':
        dwdt_func = dwdt
    else:
...
def init_single():
    """
    Resets the image between each frame so the previous frame does stay showing.
    """
    firstmass_line.set_data([],[])
    firstmass_dot.set_data([],[])
    thetaanim.set_data([],[])
    plot2anim.set_data([],[])
    energyanim.set_data([],[])
    omegaanim.set_data([],[])
    plot3anim.set_data([],[])
    return firstmass_line, firstmass_dot, thetaanim, plot2anim, energyanim, omegaanim, plot3anim
    
def animate_single(i):
    """
    This function changes the image between each frame. It changes the image for each frame. It uses the calculated data
    to create a swinging single pendulum animation.
    It also animates the graphs to show the energy and theta values of the pendulum.
    """
    firstmass_line_x = [0, x1[i%t_frames]]
    firstmass_line_y = [0, y1[i%t_frames]]
    firstmass_line.set_data([firstmass_line_x, firstmass_line_y])
    firstmass_dot_x = x1[i%t_frames]
    firstmass_dot_y = y1[i%t_frames]
    firstmass_dot.set_data([firstmass_dot_x, firstmass_dot_y])
    thetaanim_t = t[:i%t_frames]
    thetaanim_theta = theta[:i%t_frames]
    thetaanim.set_data([thetaanim_t, thetaanim_theta])
    plot2anim_t = t[:i%t_frames]
    plot2anim_true_theta = true_theta[:i%t_frames]
    plot2anim.set_data([plot2anim_t, plot2anim_true_theta]) 
    energyanim_t = t[:i%t_frames]
    energyanim_energy = Etotal[:i%t_frames]
    energyanim.set_data([energyanim_t, energyanim_energy]) 
    omegaanim_t = t[:i%t_frames]
    omegaanim_omega = omega[:i%t_frames]
    omegaanim.set_data([omegaanim_t, omegaanim_omega])
    plot3anim_t = t[:i%t_frames]
    plot3anim_true_omega = true_omega[:i%t_frames]
    plot3anim.set_data([plot3anim_t, plot3anim_true_omega]) 
    return firstmass_line, firstmass_dot, thetaanim, plot2anim, energyanim, omegaanim, plot3anim
...
def init_double():
    ...
    firstmass_line.set_data([],[])
    firstmass_dot.set_data([],[])
    secondmass_line.set_data([],[])
    secondmass_dot.set_data([],[])
    thetaanim.set_data([],[])
    plot2anim.set_data([],[])
    energyanim.set_data([],[])
    omegaanim.set_data([],[])
    plot3anim.set_data([],[])
    return firstmass_line, firstmass_dot, secondmass_line, secondmass_dot, thetaanim, plot2anim, energyanim, omegaanim, plot3anim
    
def animate_double(i):
    """
    This function changes the image between each frame. It changes the image for each frame. It uses the calculated data
    to create a swinging double pendulum animation.
    It also animates the graphs to show the energy, theta and phi values at the current time of the pendulum.
    """
    firstmass_line_x = [0, x1[i%t_frames]]
    firstmass_line_y = [0, y1[i%t_frames]]
    firstmass_line.set_data([firstmass_line_x, firstmass_line_y])
    ...
    firstmass_dot_y = y1[i%t_frames]
    firstmass_dot.set_data([firstmass_dot_x, firstmass_dot_y])
    secondmass_line_x = [x1[i%t_frames], x2[i%t_frames]]
    secondmass_line_y = [y1[i%t_frames], y2[i%t_frames]]
    secondmass_line.set_data([secondmass_line_x, secondmass_line_y])
    secondmass_dot_x = x2[i%t_frames]
    secondmass_dot_y = y2[i%t_frames]
    secondmass_dot.set_data([secondmass_dot_x, secondmass_dot_y])
    thetaanim_t = t[:i%t_frames]
    thetaanim_theta = theta[:i%t_frames]
    thetaanim.set_data([thetaanim_t, thetaanim_theta])
    plot2anim_t = t[:i%t_frames]
    plot2anim_phi = phi[:i%t_frames]
    plot2anim.set_data([plot2anim_t, plot2anim_phi]) 
    energyanim_t = t[:i%t_frames]
    energyanim_energy = Etotal[:i%t_frames]
    energyanim.set_data([energyanim_t, energyanim_energy]) 
    omegaanim_t = t[:i%t_frames]
    ...
    omegaanim.set_data([omegaanim_t, omegaanim_omega])
    plot3anim_t = t[:i%t_frames]
    plot3anim_nu = nu[:i%t_frames]
    plot3anim.set_data([plot3anim_t, plot3anim_nu]) 
    return firstmass_line, firstmass_dot, secondmass_line, secondmass_dot, thetaanim, plot2anim, energyanim, omegaanim, plot3anim
...
energyplot, = plt.plot(t, Etotal, 'g--')
energyanim, = plt.plot([],[], 'g-')
plt.title('Total energy against time')
plt.xlabel('Time')
plt.ylabel('Energy (J)')

#Subplot for the animation.
subplot3 = fig.add_subplot(2,2,3)
origin, = plt.plot(0,0, 'ko')
firstmass_line, = plt.plot(x1,y1,'b-')
firstmass_dot, = plt.plot(x1,y1,'bo')
if Type == 'Double Pendulum':
    secondmass_line, = plt.plot(x2,y2, 'r-')
    secondmass_dot, = plt.plot(x2,y2, 'ro')
plt.axis([-2*l, 2*l, -2*l, 2*l])
plt.title('Animation of the System')

#Animations
t_frames = tsize
if Type == 'Single Pendulum':
    anim=an.FuncAnimation(fig,animate_single,init_func=init_single,frames=t_frames,interval=animation_speed,blit=True)
elif Type == 'Double Pendulum':
    anim=an.FuncAnimation(fig,animate_double,init_func=init_double,frames=t_frames,interval=animation_speed,blit=True)
