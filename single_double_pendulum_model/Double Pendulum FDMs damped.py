...
method = method_list[0]  #Choose your FDM from the list
...
h = 0.01            #Ignore this if you are finding critical step-size.
...
if Type == 'Single Pendulum':
    ...
    Etotallist = [[],[],[],[]]
    for i in range(4):
        t, tsize, thetalist[i], omegalist[i], philist[i], nulist[i] = initiate(tmax, h)
        thetalist[i], omegalist[i], philist[i], nulist[i] = solveODE_double(method_list[i], tsize, omegalist[i], philist[i], thetalist[i], nulist[i], G, R, h)
        KElist[i] = 0.5*m*(omegalist[i]*l)**2 + 0.5*M*(nulist[i]*l+omegalist[i]*l)**2
        PElist[i] = m*g*l*(1-np.cos(thetalist[i])) + M*g*l*(1 - np.cos(thetalist[i]) + 1 - np.cos(philist[i]))
...
plt.legend(handles=energyplots, loc=2)
"""
...
"""
fig1 = plt.figure('Angles', figsize = (13,10))
thetaplots = []
for i in range(4):
    thetaplots += plt.plot(t, thetalist[i], colours[i], linewidth = 2, label=method_list[i])
plt.legend(handles=thetaplots, loc=2)
